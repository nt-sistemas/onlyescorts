<div class="flex flex-col w-full h-screen gap-4 p-4 bg-gray-200 lg:p-6 lg:gap-8 ">
    <div class="p-4 bg-white shadow-lg rounded-tl-3xl rounded-br-3xl">
        <div class="flex flex-col items-center h-full rounded-tl-3xl rounded-br-3xl">
            <div class="w-full lg:visible">
                <a href="{{ URL::asset(Storage::url($profile->slide)) }}" data-lightbox="Capa"
                    ...
                    <img class="object-cover w-full h-48 rounded-tl-3xl rounded-br-3xl"
                        src="{{ URL::asset(Storage::url($profile->slide)) }}" alt="{{ $profile->slug }}" />
                </a>
            </div>
            <div class="flex flex-row items-center justify-center w-full lg:visible bg-amber-700">
                <a class="flex flex-row items-center justify-center w-1/12"
                    href="{{ URL::asset(Storage::url($profile->avatar)) }}" data-lightbox="{{ $profile->name }}"
                    data-title="{{ $profile->name }}">
                    <img class="absolute object-cover object-top w-40 h-40 rounded-full shadow-lg top-52 lg:top-40 lg:h-56 lg:w-56"
                        src="{{ URL::asset(Storage::url($profile->avatar)) }}" alt="{{ $profile->slug }}" />
                </a>
            </div>
            <div class="flex flex-col items-center w-full h-full p-4 top-56 lg:flex-row lg:gap-72">
                <div class="flex flex-col items-center w-full mt-20 lg:items-start ">
                ...
                <div
                    class="flex flex-col items-center w-full h-full gap-4 p-2 lg:w-1/3 lg:justify-center lg:p-4 rounded-tl-3xl rounded-br-3xl">
                    <div
                        class="flex items-center justify-between w-2/3 gap-4 text-xl lg:w-full text-bold lg:p-2 lg:justify-center">
                        <a class="link" tooltip="Send Message for Whatsapp"
    ...
    <div class="p-4 bg-white shadow-lg h-6/12 rounded-tl-3xl rounded-br-3xl">
        ...
        <div data-te-lightbox-init class="flex flex-row flex-wrap justify-center gap-12">
            @foreach ($images as $image)
                <div class="flex flex-col justify-center object-cover w-10/12 lg:w-3/12 rounded-tl-2xl rounded-br-2xl">
                    <div>
                        @if (pathinfo($image->path, PATHINFO_EXTENSION) == 'mp4' || pathinfo($image->path, PATHINFO_EXTENSION) == 'm4v')
                            <div class="flex justify-end">
                                <div x-data="{ playing: false, muted: false }" class="">
                                    <video class="relative rounded-tl-2xl rounded-br-2xl h-full max-w-[400px]  w-full"
                                        controls x-ref="video" x-on:click="playing = !playing"
                                        x-on:play="muted = false" x-on:pause="muted = true" x-bind:autoplay="playing"
                                        x-bind:muted="muted">
                                        <source src="{{ URL::asset(Storage::url($image->path)) }}" type="video/mp4">
                                    </video>
                                </div>
                            </div>
                        @else
                            <a href="{{ URL::asset(Storage::url($image->path)) }}"
                                data-lightbox="{{ $profile->slug }}" data-title="{{ $profile->name }}"
                                class="flex justify-end">
                                <img class="relative rounded-tl-2xl rounded-br-2xl cursor-zoom-in "
                                    src="{{ URL::asset(Storage::url($image->path)) }}" alt="image">
                            </a>
                        @endif
                    </div>
                </div>
